...
int main(void)
{
    ...
    if (count == 15)
    {
        if (AMEX != 34 && AMEX != 37)  //to check AMEX 34 or 37
        {
            printf("INVALID\n");
            return 1;
        }
    ...
    if (count == 16)
    {
        if (MC == 51 || MC == 52 || MC == 53 || MC == 54 || MC == 55)  //to check if Master card first two digit
        {
            printf("MASTERCARD\n");
            return 0;
        }
    ...
    if (count == 13 && Visa == 4)  //to check for validation
    {
        printf("VISA\n");
        return 0;
    }
